...
def get_best_parameter_value_with_results(param, param_values, params_wordembeddings, params_training, tune, score, X_test, y_test):
    ...
    precision_scores = []
    recall_scores = []
    roc_auc_scores = [] # auc scores
    average_precision_scores = [] # auprc scores
    ...
    for i in range(len(param_values)):
        ...
        if tune == "wordembeddings": ####### tuning parameter for fasttext WORD EMBEDDING
            params_wordembeddings[param] = values[i]
        ...
        if tune == "training": ####### tuning parameter for fasttext TRAINING
            params_training[param] = values[i]
        ...
        ################## Evaluation
        accuracy_scores.append(metrics.accuracy_score(y_test, y_pred_ft)) # accuracy
        precision_scores.append(metrics.precision_score(y_test, y_pred_ft)) # precision
        recall_scores.append(metrics.recall_score(y_test, y_pred_ft)) # recall
        roc_auc_scores.append(metrics.roc_auc_score(y_test, y_pred_ft)) # auc
        average_precision_scores.append(metrics.average_precision_score(y_test, y_pred_ft)) # auprc
    
    if score == "auprc":
        index_max = max(range(len(average_precision_scores)), key=average_precision_scores.__getitem__)
    elif score == "auc":
        index_max = max(range(len(roc_auc_scores)), key=roc_auc_scores.__getitem__)
    elif score == "recall":
    ...
    results = {}
    results["auprc"] = average_precision_scores[index_max]
    results["auc"] = roc_auc_scores[index_max]
...
length_to_split = int(len(df_all) * 0.8)
